...
def sss(df,tf,key,data_folder,symbol, production, from_hour, to_hour):

    strategy_name = sss.__name__
    ...
    if production == 'build':
        ...
        for row in tqdm(df.iloc[200: -30].itertuples()):
            ...
            if hour > from_hour and hour < to_hour:
                ...
                result, index_end = sss_core(df,row,upper_frame_1,strategy_name, result, index_end, production, symbol)
        ...
        for row in df.iloc[-2:-1].itertuples():
            ...
            sss_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol)

def sss_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol):
        ...
        if check_heikin_candle(row) == 'bear' and check_low_ema125_bear(df,row):
            ...
            if gap < 70* tick :
                ...
                if production == 'build':
                    ...
                    lot = is_not_risky_return_lot(symbol,gap,6)
                    if lot:
                        if symbol_info.bid < buy_stop + 10*tick:

...
def adx_heikin_ema(df,tf,key,data_folder,symbol, production, from_hour, to_hour):
    
    strategy_name = adx_heikin_ema.__name__
    # print(strategy_name)
    ...
    if production == 'build' or production == 'p_build':
        ...
        for row in tqdm(df.iloc[200: -33].itertuples()):
            ...
            if hour > from_hour and hour < to_hour:
                ...
                result, index_end = adx_heikin_ema_core(df,row,upper_frame_1,strategy_name, result, index_end, production, symbol)
        ...
        for row in df.iloc[-2:-1].itertuples():
            ...
            if hour > from_hour and hour < from_hour:
                ...
                adx_heikin_ema_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol)

def adx_heikin_ema_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol):
        ...
        if check_heikin_candle(row) == 'bull' and check_low_ema125_bull(df,row):
            ...
            if tf_candle_tuple_1.close > tf_candle_tuple_1.ema_200 and tf_candle_tuple_1.atr > tf_candle_tuple_1.atr_sma_5:
                if tf_candle_tuple_1.heikin_close > tf_candle_tuple_1.heikin_open:
                    # and tf_candle_tuple_1.adx > 25 and tf_candle_tuple_1.di_p > tf_candle_tuple_1.di_n:
                    print('start buy')

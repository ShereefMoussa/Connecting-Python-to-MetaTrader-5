...
def adx_tlb_ema_atr(df,tf,key,data_folder,symbol, production, from_hour, to_hour):
    
    # strategy_name = adx_tlb_ema_atr.__name__
    ...
    if production == 'build' or production == 'p_build':
        ...
        for row in tqdm(df.iloc[200: -33].itertuples()):
            ...
            if hour > from_hour and hour < to_hour:
                ...
                result, index_end = adx_tlb_ema_atr_core(df,row,upper_frame_1,strategy_name, result, index_end, production, symbol)
        ...
        for row in df.iloc[-2:-1].itertuples():
            ...
            if hour > from_hour and hour < from_hour:
                ...
                adx_tlb_ema_atr_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol)
        
        return None, None

def adx_tlb_ema_atr_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol):
    
    if row.close > row.ema_200 and row.ema_117 > row.ema_200 and row.ema_50 > row.ema_117 :
        if check_heikin_candle(row) == 'bull' and check_low_ema125_bull(df,row) and row.tlb == 1:
            # get higher tf
            ...
            if tf_candle_tuple_1.close > tf_candle_tuple_1.ema_200:
                ...
                if tf_candle_tuple_1.heikin_close > tf_candle_tuple_1.heikin_open :
                    ...
                    stop_lose = get_stop_lose('up', df, buy_candle, tick, 14, round_dismal(row.atr,tick)/3)
    ...
    # check down 
        ...
        if check_heikin_candle(row) == 'bear' and check_low_ema125_bear(df,row) and row.tlb == -1:
            ...
            if tf_candle_tuple_1.close < tf_candle_tuple_1.ema_200:
                ...
                if tf_candle_tuple_1.heikin_close < tf_candle_tuple_1.heikin_open :
                    ...
                    stop_lose = get_stop_lose('down', df, sell_candle, tick, 14, round_dismal(row.atr,tick)/3)

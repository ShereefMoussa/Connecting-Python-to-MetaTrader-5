...
def kama_stc(df,tf,key,data_folder,symbol, production, from_hour, to_hour ,e_point):
    
    # strategy_name = kama_stc.__name__
    ...
    if production == 'build' or production == 'p_build':
        ...
        for row in tqdm(df.iloc[200: -33].itertuples()):
            ...
            if hour > from_hour and hour < to_hour:
                ...
                result, index_end = kama_stc_core(df,row,upper_frame_1,strategy_name, result, index_end, production, symbol, e_point)
        ...
        for row in df.iloc[-2:-1].itertuples():
            ...
            if hour > from_hour and hour < from_hour:
                ...
                kama_stc_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol, e_point)
        
        return None, None

def kama_stc_core(df,row,upper_frame_1,strategy_name,result,index_end, production, symbol, e_point):
    
    
    if row.kama_200 < row.kama_50 < row.kama_20 < row.close and row.low <= row.kama_20 and row.stc > 50:
        if check_heikin_candle(row) == 'bull' and check_column_bull(df, row, 'kama_20', 5, e_point[symbol]) and row.tlb == 1:
            # get higher tf
            tf_candle_tuple_1 = get_tf_candle(row.Index,upper_frame_1)

            if tf_candle_tuple_1.close > tf_candle_tuple_1.kama_200:
                # and tf_candle_tuple_1.atr > tf_candle_tuple_1.atr_smma_fast:
                # if tf_candle_tuple_1.heikin_close > tf_candle_tuple_1.heikin_open :
                    # and tf_candle_tuple_1.adx > tf_candle_tuple_1.adx_smma and tf_candle_tuple_1.di_p > tf_candle_tuple_1.di_n:
                    # print('start buy')
                    ...
                    stop_lose = get_stop_lose('up', df, buy_candle, tick, 1, round_dismal(row.atr,tick)/3)
    ...
    # check down 
    elif row.kama_200 > row.kama_50 > row.kama_20 > row.close and row.high >= row.kama_20 and row.stc > 50:
        if check_heikin_candle(row) == 'bear' and check_column_bear(df, row, 'kama_20', 5, e_point[symbol]) and row.tlb == -1:
            ...
            if tf_candle_tuple_1.close < tf_candle_tuple_1.kama_200:
                ...
                # if tf_candle_tuple_1.heikin_close < tf_candle_tuple_1.heikin_open :
                    ...
                    stop_lose = get_stop_lose('down', df, sell_candle, tick, 1, round_dismal(row.atr,tick)/3)
